...
## 11. AppArmor Hardening Plan

> **Status:** Planned (replaces `apparmor: false`)
> **Context:** `apparmor: false` was set as a stopgap after two failed attempts.

### 11.1 Root Cause of Previous Failures

| Attempt | Commit | Why It Failed |
|---------|--------|---------------|
| Custom profile | `2246701` | Written before the Python bridge (`a30672d`) — missing `/init`, `/bin/**`, `/opt/bridge-venv/**` |
| HA default profile | `1a9bbb3` | Default profile doesn't cover `/opt/**` → Python bridge fails to start |
| Disabled | `2036952` | Stopgap — no confinement |

The HA default profile works for all other apps because they don't use `/opt/`. The otelcol Python bridge (`ha-otel-bridge`) is installed in `/opt/bridge-venv/` — outside every standard Alpine and HA AppArmor path.
...
| `/bin/**` | `mrix` | Alpine busybox — all `sh` scripts — was missing |
| `/sbin/**` | `mrix` | s6 helpers |
| `/command/**` | `mrix` | s6-overlay shims |
| `/package/**` | `mr` | s6 package tree |
| `/run/s6/**` | `rwlk` | s6 runtime state |
| `/usr/bin/otelcol-contrib` | `mrix` | Main binary |
| `/opt/bridge-venv/**` | `mr` + exec | Python venv — was completely missing |
| `/opt/bridge-venv/bin/python3*` | `mrix` | Python interpreter |
| `/usr/lib/python3*/**` | `mr` | Python stdlib modules |
| `/etc/**` | `r` | System config, certs, bashio |
| `/etc/otelcol-contrib/**` | `r` | Bundled self-monitoring config |
| `/tmp/**` | `rw` | Generated otelcol YAML |
| `/config/**` | `r` | `home-assistant.log` tail |
| `/ssl/**` | `r` | TLS certificates |
| `/addon_config/**` | `rw` | Persistent add-on data |
| `/share/**` | `rw` | Shared data mount |
| `/proc/**` | `r` (owner `rw`) | Go runtime + Python runtime |
| `/sys/**` | `r` | System info |
| `/dev/null`, `/dev/urandom`, `/dev/tty`, `/dev/pts/**` | `rw` | Standard device access |
| `/run/**`, `/var/run/**` | `rw` | Unix sockets, PID files |
| `deny /var/run/docker.sock` | `rwklx` | Explicit deny — not needed |
| `network inet/inet6/unix stream/dgram` | — | OTLP export, DNS, HA API |

**Capabilities needed** (s6-overlay requires these at init):
`chown, dac_override, fowner, fsetid, kill, net_bind_service, setgid, setuid, sys_chroot`

**Capabilities denied:** `sys_admin, sys_ptrace, net_admin, sys_rawio, mknod`

### 11.3 Implementation Steps

1. **Create `otelcol/apparmor.txt`** — profile name: `hassio_otelcol`
2. **Edit `otelcol/config.yaml`** — change `apparmor: false` → `apparmor: true`
3. **Verify**: `docker build .` passes, container starts, health check (`curl http://localhost:13133/`) returns 200, `ha-bridge` service logs appear

### 11.4 Acceptance Criteria

- `docker build .` succeeds
- Container starts under AppArmor confinement (no kernel denials in `dmesg | grep apparmor`)
- Health check passes: `curl -f http://localhost:13133/`
- `ha-bridge` log line appears in container output (Python bridge started)
- `yamllint otelcol/config.yaml` passes

### 11.5 Risks

- **Unknown paths**: Go runtime or Python may access paths not listed above. If the container fails, run with `apparmor: false`, capture `dmesg | grep -i apparmor | grep DENIED`, and add the missing rules.
- **Kernel AppArmor support**: If the host kernel lacks AppArmor, setting `apparmor: true` will fail regardless. Verify with `aa-status` on the HAOS host.
